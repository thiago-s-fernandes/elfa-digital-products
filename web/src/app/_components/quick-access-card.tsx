...
export default function QuickAccessCard({ configs }: Props): React.JSX.Element {
  ...
  return (
    <Card className={cn("flex w-full py-0 border-foreground/10 rounded-md", "md:w-1/2")}>
      <CardHeader className={cn("px-6 py-4 bg-foreground rounded-t-md gap-0 border-0")}>
        <CardTitle
          className={cn("flex items-center gap-2 text-background text-lg font-bold", "lg:text-2xl")}
        >
          {icon}
          {title}
        </CardTitle>
      ...
      <CardContent>
        <CardDescription className={cn("min-h-20", "lg:text-base")}>{description}</CardDescription>
      </CardContent>
